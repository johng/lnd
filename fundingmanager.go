...
func (f *fundingManager) waitForFundingWithTimeout(
	...
	if !ch.IsInitiator {
		f.wg.Add(1)
		go f.waitForTimeout(ch, cancelChan, timeoutChan, cancelChan)
	}
...
func (f *fundingManager) waitForFundingConfirmation(
	...
	fundingBroadcastHeight := uint32(completeChan.FundingBroadcastHeight)

	confDetails, err := f.waitForTxConfirmed(txid, fundingScript,
		fundingBroadcastHeight, numConfs,
	)
